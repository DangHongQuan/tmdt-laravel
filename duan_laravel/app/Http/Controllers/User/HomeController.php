...
class HomeController extends Controller
{
   public function index()
   {
    ...
    $images=Quang_cao::all();
        return view('pages.trangchu', compact('nganhhang','viewData','images'));
   }
   ...
   public function kiemtra(Request $request){
    $kiemtra=$request->only('username','password');
    // print_r($kiemtra);
    if (auth('user_kh')->attempt($kiemtra)) {
   
        // Nếu đăng nhập thành công, chuyển hướng đến trang quản lý của admin
        // $this->middleware('admin');
        // return redirect()->action('trang-chu/dang-nhap-thanh-cong');
    }else{
...
return view('pages.danhmuc')->with('images',$images)->with('nganhhang',$nganhhang)->with('sanpham',$sanpham);
}
...
//     $folderPath = 'public/images/anhsp';
// if (!file_exists($folderPath)) {
//     mkdir($folderPath, 0777, true);
// }

// $path = $folderPath . '/' . $anh_sp->id . '.jpg';
// file_put_contents($path, $imageData);

    // foreach ($anh_sps as $anh_sp) {
    //     $path = 'public/images/' . $anh_sp->ID_ANHSP . '.jpg'; // đường dẫn để lưu ảnh
    //     $imageData = base64_decode($anh_sp->FILE_ANHSP); // Chuyển đổi base64 sang dữ liệu ảnh
    //     file_put_contents($path, $imageData); // Lưu ảnh vào đường dẫn đã chọn
    // }
